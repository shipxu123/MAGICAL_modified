...
pa_guides = [# add one compared version
...
for i, nw_params in enumerate(itertools.product(*nw_params_space)):
    for j, pa_guide in enumerate(pa_guides):
        ...
        with open("./configs/ota1_{}.json".format(count),"w") as f:
            f.write(json_obj)

        all_commomd_lines.append("mkdir ./dataset/rundir_{}\n".format(count))
        all_commomd_lines.append("python3 ../../par_flow/python/Magical.py ./configs/ota1_{}.json | tee log.txt\n".format(count, count))
