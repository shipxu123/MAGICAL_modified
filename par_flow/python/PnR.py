...
class PnR(object):
    ...
    def placeOnly(self, cktIdx, dirname, no, nw_params):
        """
        @brief PnR a circuit in the designDB
        @param the index of subckt
        """
        ...
        self.runPlace(cktIdx, dirname, no, nw_params)
        self.checkSmallModule(cktIdx)
        self.dDB.subCkt(cktIdx).isImpl = True
        print("PnR: placement finished ", self.dDB.subCkt(cktIdx).name)

    def routeOnly(self, no, pa_guide, gn_guide=None):
        """
        @brief PnR a circuit in the designDB
        @param the index of subckt
        """
        self.p.updatePlacementResult()
        self.runRoute(self.cktIdx, self.dirname, no, pa_guide, gn_guide)
        print("PnR: routing finished ", self.dDB.subCkt(self.cktIdx).name)

    def runPlace(self, cktIdx, dirname, no, nw_params):
        self.p = Placer.Placer(self.mDB, cktIdx, dirname,self.gridStep, self.halfMetWid, no, nw_params)
    ...
    def runRoute(self, cktIdx, dirname, no, pa_guide, gn_guide=None):
        ...
        # set cost guide
        if pa_guide is not None:
            assert(len(pa_guide[0]) == ckt.numNets())
            assert(len(pa_guide[1]) == ckt.numNets())
            assert(len(pa_guide[2]) == ckt.numNets())
            router.setPAGuide(pa_guide[0], pa_guide[1], pa_guide[2])
        ...
        if gn_guide is not None:
            router.parseGuide(gn_guide)
        ...
        router.writePADumb(placeFile, dirname+ckt.name+'.PADump')
        ...
        router.writePADumb(placeFile, dataset_gds_dir+ckt.name+'.PADump')
